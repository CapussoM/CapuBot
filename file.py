...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    @classmethod
    async def create_source(cls, ctx, search: str, *, loop, download=False,show=False):
...
class Music(commands.Cog):
    ...
    async def play_(self, ctx, *, search: str):
        ...
        if "open.spotify" in search:
          ...
          if "playlist" in search or "album" in search:
            urls = await spotify.get_spotify_playlist(search)
            # print(urls)
            for search in urls:
              source = None
              try:
          ...
          source = await YTDLSource.create_source(ctx, search, loop=self.bot.loop, download=False,show=True)
          await player.queue.put(source)
    ...
    @commands.command(name='makeplaylist', description="create a new playlist")
    async def makeplaylist(self, ctx):
        """Pause the currently playing song."""
        
        await ctx.send("To be implemented ⏸️")
    ...
    @commands.command(name='newplaylist', aliases=['newpl', 'newp', 'newplay'], description="create a new playlist")
    async def newplaylist(self, ctx, *, search: str):
        """Create a new playlist."""

        # player = self.get_player(ctx)
        # player.queue._queue.clear()
        if search.lower() in db.keys():
          await ctx.send('**Playlist already existing!**')
        else:
          db[search.lower()] = []
          await ctx.send('**Playlist Created!**')
    
    @commands.command(name='playlistadd', aliases=['padd', 'playadd', 'pladd'], description="add song(s) to playlist")
    async def playlistadd(self, ctx, *, search: str):
        """Add song to playlist."""


        # player = self.get_player(ctx)
        # player.queue._queue.clear()
        playlist = search.split(" ")[0]
        song = ' '.join(search.split(" ")[1:])
        print(song)
        print(playlist)
        if playlist.lower() in db.keys():
          db[playlist].append(song)
          await ctx.send('**Song Added!**')
        else:
          await ctx.send('**Playlist does not exist!**')

    @commands.command(name='showplaylist', aliases=['showp', 'sp', 'showpl'], description="show the songs of a playlist")
    async def showplaylist(self, ctx, *, search: str):
        """Show the songs of a Playlist."""
        songs = "Songs of the Playlist: " + search + "\n\n"
        if search.lower() in db.keys():
          print(db[search.lower()])
          for index,song in enumerate(db[search.lower()]):
            songs = songs + str(index+1) + ") " + song + "\n"
          await ctx.send('**' + songs + '**')
        else:
          await ctx.send('**Playlist does not exist!**')
    
    @commands.command(name='startplaylist', aliases=['startp', 'startplay', 'splay'], description="play a playlist")
    async def startplaylist(self, ctx, *, search: str):
        """Play the songs of a Playlist."""
    ...
    @commands.command(name='showplaylists', aliases=['showps', 'showpls', 'showplay'], description="show playlists")
